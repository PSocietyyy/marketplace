    ...
    <div class="mb-5">
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div>
                    <div class="relative">
                        <button type="button" wire:click="sortBy('umkn_name')" 
            ...
            <div class="flex justify-between items-center mb-4">
                <div class="text-sm text-gray-600">
                    <span class="font-medium">{{ $umkns->total() }} Pengajuan</span>
                    @if ($search)
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Alamat</th>
                                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($umkns as $umkn)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="w-16 h-16 bg-gray-100 flex items-center justify-center overflow-hidden rounded">
                                            @if (!empty($umkn->logo))
                                                <img src="{{ asset('storage/'.$umkn->logo)}}" alt="{{ $umkn->umkn_name }}" class="object-cover w-full h-full">
                                            @else
                                                <i class="ri-image-line text-3xl text-gray-400"></i>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap max-w-xs truncate" title="{{ $umkn->umkn_name }}">
                                        {{ $umkn->umkn_name }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{ $umkn->number_phone }}
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="font-medium">{{ $umkn->description }}</span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center space-x-1">
                                        <x-button 
                                            variant="success" 
                                            size="xs" 
                                            wire:click="approvedUmkn({{ $umkn->id }})" 
                                            icon="ri-eye-line"
                                            iconPosition="left"
                                        >
                                            Terima
                                        ...
                                        <x-button 
                                            variant="danger" 
                                            size="xs" 
                                            wire:click="rejectedUmkn({{ $umkn->id }})" 
                                            icon="ri-eye-line"
                                            iconPosition="left"
                                        >
                                            Tolak
            ...
            @else
                <div class="text-center py-16">
                    <div class="max-w-md mx-auto">
                        ...
                        <h3 class="text-xl font-medium text-gray-900 mb-3">Umkn Tidak Ditemukan</h3>
                        <p class="text-gray-600 mb-6 leading-relaxed">Coba ubah kata kunci pencarian atau filter yang Anda gunakan untuk menemukan umkn yang sesuai</p>
                        @if ($search)
                            <x-button 
                                variant="primary" 
                                wire:click="clearFilters"
